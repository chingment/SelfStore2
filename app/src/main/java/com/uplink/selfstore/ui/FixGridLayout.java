...
public class FixGridLayout extends ViewGroup {
    ...
    private String tag="FixGridLayout";
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        Log.d(tag,String.format("离左边:%s,离右边:%s",l,r));
        ...
        int _avgCellWidth=r/columns;

        Log.d(tag,String.format("平均宽度:%s",_avgCellWidth));
        ...
        Log.d(tag,String.format("总的数量:%s",count));

        for (int j = 0; j < count; j++) {
            final View childView = getChildAt(j);

            LayoutParams params = (LayoutParams) childView.getLayoutParams();
            params.width=_avgCellWidth;
